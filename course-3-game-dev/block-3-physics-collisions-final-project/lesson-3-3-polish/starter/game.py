...
pygame.init()
# TODO: Init the mixer so Sound can play (safe even if files are missing later)
# pygame.mixer.init()
...
# TODO: Load catch.wav / miss.wav with try/except (optional sounds)
catch_sound = None
miss_sound = None
# try:
#     pygame.mixer.init()
...
score = 0
# TODO: Track the best score this session (in memory)
high_score = 0
misses = 0
MAX_MISSES = 3
...
while running:
    ...
    if state == START:
        ...
        # TODO: Difficulty ramp — fall faster as score grows
        ...
        if paddle.colliderect(star):
            score += 1
            # TODO: play_safe(catch_sound)
            reset_star()
        elif star.top > HEIGHT:
            misses += 1
            # TODO: play_safe(miss_sound)
            reset_star()
            if misses >= MAX_MISSES:
                # TODO: Update high_score if this round beat it
                # if score > high_score:
                #     high_score = score
                state = GAME_OVER
